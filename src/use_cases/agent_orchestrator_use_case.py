...
from src.domain.ports import ILLMService, IRagRepository, IMcpToolClient
...
class AgentOrchestratorUseCase:
    ...
    def reasoning_node(self, state: AgentState) -> dict:
        ...
        return self.llm.generate_response(state)
    ...
    async def mcp_tool_node(self, state: AgentState) -> dict:
        print("🛠️ Ejecutando herramienta MCP...")
    ...
    def router_node(self, state: AgentState) -> str:
        ...
        palabra_clave_rag = ["pdf","empleo","empleos,","trabajo","trabajos","curriculum","cv","currículum","curriculo","curriculos","currículos", "Colombia","IA","Trabajo IA", "Vacantes","vacante","vacantes","vacancia","vacancias","oferta laboral","ofertas laborales","oferta de trabajo","ofertas de trabajo","oferta de empleo","ofertas de empleo"]

        # Si el mensaje del LLM contiene la orden de usar herramientas (tool_calls)
        if last_message.tool_calls:
            print("   -> 🔀 El LLM decidió usar una herramienta. Redirigiendo a MCP...")
    ...
    def compile_graph(self):
        ...
        graph.add_node("mcp_tool_node", self.mcp_tool_node)
        ...
        graph.add_conditional_edges(
            ...
            self.router_node, # La función que decide el camino
            {
                ...
                "mcp_tool_node": "mcp_tool_node", # Si el router devuelve "mcp_tool_node", ve allí
        ...
        graph.add_edge("mcp_tool_node", "reasoning_node")
